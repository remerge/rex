...
// slightly modified from https://github.com/bitly/nsq/blob/master/nsqd/diskqueue.go
...
func NewDiskQueue(name string, dataPath string, maxBytesPerFile int64, syncEvery int64, syncTimeout time.Duration) *DiskQueue {
	d := DiskQueue{
		name:              name,
		dataPath:          dataPath,
		...
		log:               loggo.GetLogger("rex.diskqueue." + name),
	}
...
func (d *DiskQueue) readOne() ([]byte, error) {
	...
	if d.readFile == nil {
		...
		d.log.Infof("readOne() opened %s", curFileName)
...
func (d *DiskQueue) writeOne(data []byte) error {
	...
	if d.writeFile == nil {
		...
		d.log.Infof("writeOne() opened %s", curFileName)
...
func (d *DiskQueue) handleReadError() {
	...
	d.log.Infof("jump to next file and saving bad file as %s", badRenameFn)
